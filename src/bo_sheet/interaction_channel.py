__doc__ = """For randomly changed channel walls"""

"""
This script contains classes and functions considering friction acting on sheet-shaped robots. The friction comes from
the substrate and wall of the channel.
In this version, the channel shape is defined as a shapely object. In this way, we give more freedom to control the
channel shape.
...
def _truncated_normal_sample_std1(loc: float, scale: float) -> float:
    ...
    # Standard normal bounds (-1 and 1 because we want ±1 standard deviation)
    alpha = -1.0
    beta = 1.0
...
def _fric_vari_1std(
    ...
    """
    This function is intended for computing all the friction coefficients' variation at once.\\
    Variation is limited to ±1 standard deviation.\\
    If the indexed fric coeff - variation_level < 0, then the std is set to be fric coeff * variation_level.\\
    This ensures positive definiteness.\n

    Kinetic friction coeffcients <= static friction coefficients of the same tpye.\n

    Returns:
        (kinetic_mu_substrate_array, static_mu_substrate_array, kinetic_mu_wall_array, static_mu_wall_array)
    """
    for i in prange(kinetic_mu_wall_array.shape[0]):
        if kinetic_mu_substrate_array[i] - variation_level > 0:
            kinetic_mu_substrate_array[i] = _truncated_normal_sample_std1(
                loc=kinetic_mu_substrate_array[i],
                scale=variation_level
            )
        else:
            kinetic_mu_substrate_array[i] = _truncated_normal_sample_std1(
                loc=kinetic_mu_substrate_array[i],
                scale=kinetic_mu_substrate_array[i] * variation_level
            )
        
        if static_mu_substrate_array[i] - variation_level > 0:
            static_mu_substrate_array[i] = _truncated_normal_sample_std1(
                loc=static_mu_substrate_array[i],
                scale=variation_level
            )
        else:
            static_mu_substrate_array[i] = _truncated_normal_sample_std1(
                loc=static_mu_substrate_array[i],
                scale=static_mu_substrate_array[i] * variation_level
            )

        if kinetic_mu_wall_array[i] - variation_level > 0:
            kinetic_mu_wall_array[i] = _truncated_normal_sample_std1(
                loc=kinetic_mu_wall_array[i],
                scale=variation_level
            )
        else:
            kinetic_mu_wall_array[i] = _truncated_normal_sample_std1(
                loc=kinetic_mu_wall_array[i],
                scale=kinetic_mu_wall_array[i] * variation_level
            )
        
        if static_mu_wall_array[i] - variation_level > 0:
            static_mu_wall_array[i] = _truncated_normal_sample_std1(
                loc=static_mu_wall_array[i],
                scale=variation_level
            )
        else:
            static_mu_wall_array[i] = _truncated_normal_sample_std1(
                loc=static_mu_wall_array[i],
                scale=static_mu_wall_array[i] * variation_level
            )

    if np.all(kinetic_mu_substrate_array > static_mu_substrate_array):
        kinetic_mu_substrate_array = static_mu_substrate_array
    if np.all(kinetic_mu_wall_array > static_mu_wall_array):
        kinetic_mu_wall_array = static_mu_wall_array

    return (kinetic_mu_substrate_array, static_mu_substrate_array, kinetic_mu_wall_array, static_mu_wall_array)

# =========================================Friction with the substrate==================================================
# Only applies normal force and friction along axial direction for a surface change in x-y plane
# The surface extends to infinite in z direction
# No torque considered (Here it means the torque effect from friction is neglected)
class ChannelInteraction(NoForces):
    def __init__(
        ...
        self.dynamic_fric_coeff = dynamic_fric_coeff
        self.dynamic_fric_variation_level = dynamic_fric_variation_level

        # Adopting Prof. Hu's suggestion of varying around a fixed value.
        self.original_kinetic_mu_substrate_array = kinetic_mu_substrate_array.copy()
        self.original_static_mu_substrate_array = static_mu_substrate_array.copy()
        self.original_kinetic_mu_wall_array = kinetic_mu_wall_array.copy()
        self.original_static_mu_wall_array = static_mu_wall_array.copy()

        # Pre-compute channel boundary coordinates
        ...
        # Should vary but only at initialization.
        # See the channel_force() function for should vary and at every time step.
        if self.variation_level > 0.0 and self.dynamic_fric_coeff == False:
            self.kinetic_mu_substrate_array, self.static_mu_substrate_array, self.kinetic_mu_wall_array, self.static_mu_wall_array = _fric_vari_1std(
                self.kinetic_mu_substrate_array,
                self.static_mu_substrate_array,
                self.kinetic_mu_wall_array,
                self.static_mu_wall_array,
                self.variation_level
            )
    ...
    def apply_forces(self, system: elastica.CosseratRod, time=0.0):
        return channel_force(
            ...
            self.dynamic_fric_coeff,
            self.dynamic_fric_variation_level,
            self.original_kinetic_mu_substrate_array,
            self.original_static_mu_substrate_array,
            self.original_kinetic_mu_wall_array,
            self.original_static_mu_wall_array
        )
...
def channel_force(
    ...
    dynamic_fric_coeff: bool,
    dynamic_fric_variation_level: float,
    original_kinetic_mu_substrate_array: npt.NDArray[np.float64],
    original_static_mu_substrate_array: npt.NDArray[np.float64],
    original_kinetic_mu_wall_array: npt.NDArray[np.float64],
    original_static_mu_wall_array: npt.NDArray[np.float64]
):
    """
    ...
    substrate_friction_ndarray = compute_friction_force_substrate_array(
        ...
        velocity_threshold,
        dynamic_fric_coeff,
        dynamic_fric_variation_level,
        original_kinetic_mu_substrate_array,
        original_static_mu_substrate_array
    )
    ...
    wall_friction_array = compute_friction_force_wall_array(
        ...
        dynamic_fric_coeff,
        dynamic_fric_variation_level,
        original_kinetic_mu_wall_array,
        original_static_mu_wall_array
    )

    update_nodal_force(wall_friction_array, external_forces)

# 11/7/2025 CHANGE: Removed compute_friction_force_wall() and compute_friction_force_substrate() for being superseded by
# compute_friction_force_wall_array() and compute_friction_force_substrate_array()
...
def compute_friction_force_wall_array(position_collection: npt.NDArray[np.float64],
    ...
    for i in prange(N):
        ...
        if dynamic_fric_variation_level > 0.0 and dynamic_fric_coeff:
            if original_kinetic_mu_wall_array[i] - dynamic_fric_variation_level > 0.0:
                kinetic_coeff = _truncated_normal_sample_std1(
                    loc=original_kinetic_mu_wall_array[i],
                    scale=dynamic_fric_variation_level
                )
            else:
                kinetic_coeff = _truncated_normal_sample_std1(
                    loc=original_kinetic_mu_wall_array[i],
                    scale=original_kinetic_mu_wall_array[i] * dynamic_fric_variation_level
                )

            if original_static_mu_wall_array[i] - dynamic_fric_variation_level > 0.0:
                static_coeff = _truncated_normal_sample_std1(
                    loc=original_static_mu_wall_array[i],
                    scale=dynamic_fric_variation_level
                )
            else:
                static_coeff = _truncated_normal_sample_std1(
                    loc=original_static_mu_wall_array[i],
                    scale=original_static_mu_wall_array[i] * dynamic_fric_variation_level
                )
        else:
            kinetic_coeff: float = kinetic_mu_wall_array[i]
            static_coeff: float = static_mu_wall_array[i]
...
def compute_friction_force_substrate_array(position_collection: npt.NDArray[np.float64],
    ...
    for i in prange(N):
        if dynamic_fric_variation_level > 0.0 and dynamic_fric_coeff:
            if original_kinetic_mu_substrate_array[i] - dynamic_fric_variation_level > 0.0:
                kinetic_coeff = _truncated_normal_sample_std1(
                    loc=original_kinetic_mu_substrate_array[i],
                    scale=dynamic_fric_variation_level
                )
            else:
                kinetic_coeff = _truncated_normal_sample_std1(
                    loc=original_kinetic_mu_substrate_array[i],
                    scale=original_kinetic_mu_substrate_array[i] * dynamic_fric_variation_level
                )

            if original_static_mu_substrate_array[i] - dynamic_fric_variation_level > 0.0:
                static_coeff = _truncated_normal_sample_std1(
                    loc=original_static_mu_substrate_array[i],
                    scale=dynamic_fric_variation_level
                )
            else:
                static_coeff = _truncated_normal_sample_std1(
                    loc=original_static_mu_substrate_array[i],
                    scale=original_static_mu_substrate_array[i] * dynamic_fric_variation_level
                )
        else:
            kinetic_coeff: float = kinetic_mu_substrate_array[i]
            static_coeff: float = static_mu_substrate_array[i]
        
        # Compute kinetic or static friction.
        if v_mags[i] >= velocity_threshold:
            # Kinetic friction
...
def compute_normal_force_array_fast(position_collection: npt.NDArray[np.float64],
    ...
    """
    # Unwrap the 3×N arrays into 1D arrays
